...
pub fn handle_play(
    ...
    WINNING_COMBINATION.save(deps.storage, &state.lottery_counter.to_be_bytes(), &winning_combination.to_string())?;
    ...
    JACKPOT.save(deps.storage, &state.lottery_counter.to_be_bytes(), &jackpot_after)?;
    // Update the state
    ...
    Ok(Response {
        ...
        data: None,
        attributes: vec![attr("action", "reward"), attr("by", info.sender.to_string()), attr("to", res.worker)]
    })
}
...
pub fn handle_claim(
    ...
    let lottery_winning_combination = match WINNING_COMBINATION.may_load(deps.storage, &last_lottery_counter_round.to_be_bytes())?
    {
    ...
    match addresses {
        None => {
            match PREFIXED_USER_COMBINATION.may_load(deps.storage, (&last_lottery_counter_round.to_be_bytes(), addr.as_slice()))?
            {
                None => {}
    ...
    for (addr, comb_raw) in combination {
        match PREFIXED_WINNER.may_load(deps.storage, (&last_lottery_counter_round.to_be_bytes(), addr.as_slice()))?
        {
            None => {
    ...
    if some_winner == 0 {
        return Err(StdError::generic_err("No winning combination or already claimed"));
    }
...
pub fn handle_collect(
    ...
    PREFIXED_WINNER.save(deps.storage, (&last_lottery_counter_round.to_be_bytes(), canonical_addr.as_slice()), &rewards)?;
    ...
    Ok(Response {
        ...
        data: None,
        attributes: vec![attr("action", "handle_collect"), attr("by", info.sender), attr("to", addr), attr("prize_collect", "true")]
    })
}
    ...
    let proposal_type = if let Proposal::HolderFeePercentage = proposal {
        ...
        match amount {
            Some(amount) => {
                ...
                let msg_balance = Cw20QueryMsg::Balance { address: env.contract.address.to_string() };
                let loterra_human =  deps
...
pub fn handle_vote(
    ...
    PREFIXED_POLL_VOTE.update(deps.storage, (&poll_id.to_be_bytes(), &sender.as_slice()), |exist| match exist  {
        None => Ok(approve),
...
pub fn handle_present_proposal(
    ...
    if poll.weight_yes_vote.add(poll.weight_no_vote).u128() * 100 / total_weight_bonded.u128()
        < 50
    {
...
mod tests {
    ...
    mod play {
        ...
        fn success() {
            ...
            let state = read_state(deps.as_ref().storage).unwrap();
            JACKPOT.save(deps.as_mut().storage, &(state.lottery_counter - 1).to_be_bytes(), &Uint128::zero());
            ...
            let store =
                WINNING_COMBINATION.load(deps.as_ref().storage, &state.lottery_counter.to_be_bytes()).unwrap();
            assert_eq!(store, "39493d");
            let state_after = read_state(deps.as_ref().storage).unwrap();
            let jackpot_reward_after =  JACKPOT.load(deps.as_ref().storage, &state.lottery_counter.to_be_bytes()).unwrap();
        ...
        fn success_no_big_winner() {
            ...
            execute(deps.as_mut(), env.clone(), mock_info(before_all.default_sender_two.as_str().clone(),
                                                         &[Coin {
                                                             denom: "ust".to_string(),
                                                             amount: Uint128(1_000_000),
                                                         }]),msg.clone()).unwrap();
            ...
            let state_after = read_state(deps.as_ref().storage).unwrap();
            let jackpot_reward_after =
                JACKPOT.load(deps.as_ref().storage,&state.lottery_counter.to_be_bytes()).unwrap();
        ...
        fn success_bonus_holder_end_fee_superior_20_percent() {
            ...
            execute(deps.as_mut(), env.clone(), mock_info(
            ...
            execute(deps.as_mut(), env, mock_info(
